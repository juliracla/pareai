...
export default function Security() {
  ...
  return (
    <section className="py-24 bg-slate-950" id="security">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="order-2 lg:order-1">
            <div className="grid grid-cols-2 gap-4">
              {items.map((item, idx) => (
                <div 
                  key={idx} 
                  className={`reveal reveal-delay-${idx + 1} bg-slate-900 p-6 rounded-2xl flex flex-col items-center text-center border border-transparent hover:border-[#0ed90e]/20 transition-all shadow-sm`}
                >
                  <div className="mb-4">{item.icon}</div>
                  <h4 className="font-bold mb-1 text-white uppercase italic">{item.title}</h4>
          ...
          <div className="order-1 lg:order-2 flex flex-col gap-6 text-left">
            <h2 className="text-3xl lg:text-5xl font-black tracking-tight leading-tight text-white uppercase italic">
