...
export default function Pricing() {
  ...
  return (
    <section className="py-24 bg-[#f6f7f8] dark:bg-[#111821] overflow-hidden relative" id="pricing">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl lg:text-5xl font-black mb-6 tracking-tight dark:text-white uppercase italic text-slate-900">
          ...
          {plans.map((plan, idx) => {
            ...
            return (
              <div 
                ...
                className={`reveal reveal-delay-${idx + 1} cursor-pointer bg-white dark:bg-slate-900 p-8 rounded-3xl border-2 transition-all duration-500 relative text-left 
                  ${isSelected 
                    ? 'border-[#0ed90e] shadow-[0_0_40px_-10px_rgba(14,217,14,0.4)] scale-105 z-10' 
                    : 'border-slate-200 dark:border-slate-800 shadow-sm hover:border-[#0ed90e]/50'
                  }`}
              >
                {plan.popular && (
                  <div className={`absolute -top-4 left-1/2 -translate-x-1/2 text-[#111821] text-[10px] font-black px-4 py-1 rounded-full uppercase tracking-widest transition-colors duration-500 ${isSelected ? 'bg-[#0ed90e]' : 'bg-slate-400 dark:bg-slate-700'}`}>
                    Más popular
                  </div>
                )}
                <h3 className="text-xl font-bold mb-1 dark:text-white uppercase italic">{plan.name}</h3>
                ...
                <ul className="space-y-4 mb-8 flex-grow">
                  {plan.features.map((feature, fIdx) => (
                    <li key={fIdx} className={`flex items-center gap-3 text-sm font-semibold ${feature.available ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}>
                      {feature.available ? (
                ...
                <button 
                  className={`w-full py-4 rounded-xl font-black transition-all duration-500 ${isSelected ? 'bg-[#0ed90e] text-[#111821] shadow-lg shadow-[#0ed90e]/30' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}
                >
